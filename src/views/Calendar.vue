...
const monthDays = computed(function() {
...
const checkinCount = computed(function() {
  return checkinDays.value.filter(function(d) {
    const date = new Date(d)
    return date.getFullYear() === currentYear.value && date.getMonth() === currentMonth.value
  }).length
})

const currentMonthStr = computed(function() {
    ...
    <div class="bg-white rounded-xl p-6">
      ...
      <div class="grid grid-cols-7 gap-2">
        <div
          ...
          :key="index"
          :class="[
            'aspect-square rounded-lg flex items-center justify-center text-sm transition-all cursor-pointer',
